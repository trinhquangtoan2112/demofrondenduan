...
export default function AllStories() {
  ...
  useEffect(() => {
    const filtered = selectedGenre
      ? list.filter((story) => story.tenTheLoai === selectedGenre)
      : list;
  ...
  const currentStories = filteredList.slice(
    indexOfFirstStory,
    indexOfLastStory
  );
        ...
        {currentStories.length > 0 ? (
          currentStories.map((data) => (
            ...
            >
              ...
              <p className="text-center font-medium mb-2 truncate">
                {data.tenTruyen}
              </p>{" "}
              {/* Cắt ngắn tiêu đề */}
